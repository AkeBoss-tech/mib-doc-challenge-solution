#!/usr/bin/env python3
"""Train one output-only visible-OCR categorical model."""
...
def exported_probabilities(text: str, features, intercepts) -> np.ndarray:
    weighted = []
...
def main() -> None:
    ...
    labels = [truth[case_id][args.field] for case_id in ids]
    ...
    features = {
        name: (float(vectorizer.idf_[index]), tuple(float(value) for value in classifier.coef_[:, index]))
        for index, name in enumerate(names)
    }
    intercepts = tuple(float(value) for value in classifier.intercept_)
    expected = classifier.predict_proba(matrix)
    observed = np.stack([exported_probabilities(text, features, intercepts) for text in texts])
    ...
    payload = {
        "schema": f"akeboss-visible-ocr-{args.field.replace('_', '-')}/v1",
        "field": args.field,
    ...
    print(args.field, "features", len(features), "maximum_export_error", maximum_error)
